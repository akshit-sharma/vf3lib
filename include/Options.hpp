...
#ifdef VF3P
	outstring +=
		"-c [start cpu] -t [# of threads] -a [version id] -h [SSR high "
		"limit] -l [local stack limit] -k ";
#endif
...
std::unique_ptr<vflib::MatchingEngine<state_t>>
	CreateMatchingEngine(const Options &opt)
{
#ifdef VF3P
	switch (opt.algo) {
	case VF3PGSS:
		return std::make_unique<vflib::ParallelMatchingEngine<state_t>>(
			opt.numOfThreads, opt.storeSolutions, opt.lockFree, opt.cpu);
	case VF3PWLS:
		return std::make_unique<vflib::ParallelMatchingEngineWLS<state_t>>(
			opt.numOfThreads,
			opt.storeSolutions,
			opt.lockFree,
			opt.cpu,
