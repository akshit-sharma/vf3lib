...
typedef uint8_t nodeDir_t;
const nodeDir_t NODE_DIR_NONE = 0;
const nodeDir_t NODE_DIR_IN = 1;
const nodeDir_t NODE_DIR_OUT = 2;
const nodeDir_t NODE_DIR_BOTH = 3;

/*----------------------------------------------------------
 * @class VF3State
 * @brief A representation of the SSR current state
 * @details The class is used for graph isomorphism
 ---------------------------------------------------------*/
template<
	typename Node1,
	typename Node2,
...
private:
	...
	nodeID_t
		*predecessors;// Previous node in the ordered sequence connected to a node

	// TERMINAL SET SIZE
	// BE AWARE: Core nodes are also counted by these
	// GLOBAL SIZE
	uint32_t t2in_len, t2both_len,
		t2out_len;// Len of Terminal set for the second graph
	uint32_t *t1in_len, *t1both_len,
		*t1out_len;// Len of Terminal set for the first graph for each level
							 // SIZE FOR EACH CLASS
	uint32_t *t2both_len_c, *t2in_len_c,
		*t2out_len_c;// Len of Terminal set for the second graph for each class
	uint32_t **t1both_len_c, **t1in_len_c,
		**t1out_len_c;// Len of Terminal set for the first graph for each class
									// end level
...
public:
	...
	bool NextPair(
		nodeID_t *pn1,
		nodeID_t *pn2,
		nodeID_t prev_n1 = NULL_NODE,
		nodeID_t prev_n2 = NULL_NODE);
...
VF3State<
	...
	EdgeComparisonFunctor>::
	VF3State(
		ARGraph<Node1, Edge1> *ag1,
		ARGraph<Node2, Edge2> *ag2,
		uint32_t *class_1,
		uint32_t *class_2,
		uint32_t nclass,
		nodeID_t *order,
	...
	: State(ag1->NodeCount(), ag2->NodeCount(), order, induced)
{
	...
	t1both_len = new uint32_t[n1 + 1];
	t1in_len = new uint32_t[n1 + 1];
	t1out_len = new uint32_t[n1 + 1];

	termin1 = (uint32_t *)calloc(n1, sizeof(uint32_t));
	termout1 = (uint32_t *)calloc(n1, sizeof(uint32_t));
	new1 = (uint32_t *)calloc(n1, sizeof(uint32_t));

	t1both_len_c = (uint32_t **)malloc((n1 + 1) * sizeof(uint32_t *));
	t1in_len_c = (uint32_t **)malloc((n1 + 1) * sizeof(uint32_t *));
	t1out_len_c = (uint32_t **)malloc((n1 + 1) * sizeof(uint32_t *));

	termin1_c = (uint32_t **)malloc(n1 * sizeof(uint32_t *));
	termout1_c = (uint32_t **)malloc(n1 * sizeof(uint32_t *));
	new1_c = (uint32_t **)malloc(n1 * sizeof(uint32_t *));

	core_len_c = (uint32_t *)calloc(classes_count, sizeof(uint32_t));
	t2both_len_c = (uint32_t *)calloc(classes_count, sizeof(uint32_t));
	t2in_len_c = (uint32_t *)calloc(classes_count, sizeof(uint32_t));
	t2out_len_c = (uint32_t *)calloc(classes_count, sizeof(uint32_t));
...
VF3State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::VF3State(const VF3State &state)
	: State(state)
{
	g1 = state.g1;
	g2 = state.g2;
...
VF3State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::~VF3State()
{

	if (--*share_count > 0) BackTrack();
...
// Provare ad avere in1 ed ou1 predeterminati, senza doverlo calcolare ad ogni
// iterazione La loro dimensione ad ogni livello dell'albero di ricerca e'
// predeterminato In questo modo mi basta conoscere solo l'ordine di scelta e la
// dimensione di in1 ed out1
template<
	typename Node1,
	typename Node2,
	typename Edge1,
...
void VF3State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::ComputeFirstGraphTraversing()
{
	...
	NextPair(nodeID_t *pn1, nodeID_t *pn2, nodeID_t prev_n1, nodeID_t prev_n2)
{
	...
	if (predecessors[curr_n1] != NULL_NODE) {
		...
		if (last_candidate_index >= pred_set_size) return false;
		...
		while (prev_n2 < n2
					 && (core_2[prev_n2] != NULL_NODE || class_2[prev_n2] != c)) {
			prev_n2++;
		}
	}
	...
	EdgeComparisonFunctor>::IsFeasiblePair(nodeID_t node1, nodeID_t node2)
{
	...
	if (!nf(g1->GetNodeAttr(node1), g2->GetNodeAttr(node2))) return false;

	if (
		g1->InEdgeCount(node1) != g2->InEdgeCount(node2)
...
void VF3State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::BackTrack()
{
	...
	if (orig_core_len < core_len) {
		...
		if (in_2[node2] == core_len) {
			...
			t2in_len_c[node_c]--;
			if (out_2[node2]) t2both_len_c[node_c]--;
		}
		...
		if (out_2[node2] == core_len) {
			...
			t2out_len_c[node_c]--;
			if (in_2[node2]) t2both_len_c[node_c]--;
		}
		...
		for (i = 0; i < g2->InEdgeCount(node2); i++) {
			...
			if (in_2[other] == core_len) {
				...
				t2in_len_c[other_c]--;
				if (out_2[other]) t2both_len_c[other_c]--;
			}
		}
		...
		for (i = 0; i < g2->OutEdgeCount(node2); i++) {
			...
			if (out_2[other] == core_len) {
				...
				t2out_len_c[other_c]--;
				if (in_2[other]) t2both_len_c[other_c]--;
			}
		}
...
bool VF3State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::IsDead() const
{

	if (
		t1both_len[core_len] != t2both_len || t1out_len[core_len] != t2out_len
		|| t1in_len[core_len] != t2in_len) {
		return true;
	}

	for (int32_t c = 0; c < classes_count; c++) {
		if (
			t1both_len_c[core_len][c] != t2both_len_c[c]
