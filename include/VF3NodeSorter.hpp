...
public:
	...
	{
		if (degM > rhs.degM) return true;

		if (degM == rhs.degM && probability < rhs.probability) return true;

		if (degM == rhs.degM && probability == rhs.probability && deg > rhs.deg)
			return true;
 ...
 * @brief Compares VF3SortingNode using the propability. If the probability of
 * two nodes is equal uses the degree.
 */
class CompareSortingNodeProbability
{
public:
	bool operator()(VF3SortingNode *n1, VF3SortingNode *n2)
	{
		...
		if (
			n1->GetProbability() == n2->GetProbability()
			&& n1->GetDegree() > n2->GetDegree())
			return true;
		return false;
	}
};
...
private:
	...
	void AddNodeToSortedSet(
		ARGraph<Node, Edge> *pattern,
		nodeID_t node,
		nodeID_t level,
		std::vector<VF3SortingNode *> &nodes,
		std::vector<VF3SortingNode *> &candidates,
		std::vector<nodeID_t> &nodes_order)
	{
		...
		for (i = 0; i < in1_count; i++) {
			...
			if (!nodes[neigh]->IsUsed()) nodes[neigh]->IncreaseMatchingDegree();
...
public:
	...
	~VF3NodeSorter() { delete probability; }
	...
	std::vector<nodeID_t> SortNodes(ARGraph<Node, Edge> *pattern)
	{
		...
		std::vector<nodeID_t>
			nodes_order;// Output vector with sorted nodes
									// We use two structures the fist used to quickly edit the
									// deg_m of a node by its index The second to perform a
									// priority queue by means a max heap
		...
		for (i = 0; i < nodeCount; i++) {
			nodes[i] = new VF3SortingNode(
				i, pattern->EdgeCount(i), probability->GetProbability(pattern, i));
		}

		uint32_t n = 0;
		candidate_it = std::min_element(
			nodes.begin(), nodes.end(), CompareSortingNodeProbability());
