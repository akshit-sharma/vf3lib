...
namespace vflib {
template<typename Node, typename Edge> class NodeProbabilityStrategy
{
...
template<typename Node, typename Edge> class UniformProbability
{
public:
...
{
public:
	IsoNodeProbability(ARGraph<Node, Edge> *source)
		: NodeProbabilityStrategy<Node, Edge>(source)
	{}
	...
	double GetProbability(ARGraph<Node, Edge> *g, nodeID_t id)
	{
		...
		node_p = this->out_deg[node_out_deg] * this->in_deg[node_in_deg]
						 * this->degree[node_deg] * this->labels[node_attr];
		return node_p;
	}
};
...
{
public:
	SubIsoNodeProbability(ARGraph<Node, Edge> *source)
		: NodeProbabilityStrategy<Node, Edge>(source)
	{}
	...
	double GetProbability(ARGraph<Node, Edge> *g, nodeID_t id)
	{
		...
		node_p =
			out_deg_p_sum * in_deg_p_sum * tot_deg_p_sum * this->labels[node_attr];
