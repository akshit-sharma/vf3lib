 ...
 */
template<typename Node, typename Edge> class NodeSorter
{
public:
 ...
 * @details Mainains the node insertion order (as they have been inserted in the
 * graph)
 */
...
public:
	std::vector<nodeID_t> SortNodes(ARGraph<Node, Edge> *pattern)
	{
		...
		for (i = 0; i < pattern->NodeCount(); i++) sorted.push_back(i);
		return sorted;
	}
};
...
public:
	SortingNode(nodeID_t id, uint32_t deg = 0)
		: id(id), deg(deg), used(false), inCandidate(false)
	{}
	...
	bool operator==(const SortingNode &rhs) const { return id == rhs.id; }

	bool operator<(const SortingNode &rhs) const { return true; }
};

/*
 * @class CompareCandidates
 * @brief Functor used to compare the candidate node on the base of the used
 * criteria
 */
template<typename Node> class CompareCandidates
{
public:
...
{
public:
	bool operator()(SortingNode *node) { return !node->IsUsed(); }
};
...
public:
	std::vector<nodeID_t> SortNodes(ARGraph<Node, Edge> *pattern)
	{
		...
		for (i = 0; i < nodeCount; i++) { nodes[i] = new SortingNode(i); }
