 ...
 * @details Extends this class to add action to perfom when a new solution is
 *found The Matching vistor must be passed to the constructor of the Matching
 *Engine to be used MatchingEngine e = new MatchingEngine(visitor)
 */
class MatchingVisitor
{
public:
	 ...
	 * @details A match visitor is a function that is invoked for each match that
	 * has been found. If the function returns FALSE, then the next match is
...
template<typename VFState> class MatchingEngine
{
protected:
	/*
...
public:
	MatchingEngine(bool storeSolutions = false)
		: visit(NULL), solCount(0), storeSolutions(storeSolutions){};
	...
	virtual void ResetSolutionCounter() { solCount = 0; }

	inline struct timeval GetFirstSolutionTime() { return fist_solution_time; }
	...
	/**
	 * @brief  Finds a matching between two graph, if it exists, given the initial
	...
	virtual bool FindFirstMatching(VFState &s)
	{
		...
		while (!found && s.NextPair(&n1, &n2, n1, n2)) {
			...
			uint64_t child_id =
				trace->AppendCandidateState(s.GetTraceID(), current_depth + 1);
	...
	virtual bool FindAllMatchings(VFState &s)
	{
		...
		if (s.IsGoal()) {
			...
			if (!solCount) gettimeofday(&fist_solution_time, NULL);
		...
		while (s.NextPair(&n1, &n2, n1, n2)) {
			...
			if (s.IsFeasiblePair(n1, n2)) {
				...
				if (FindAllMatchings(s1)) { return true; }
			}
		}
