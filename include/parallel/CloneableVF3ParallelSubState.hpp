...
#ifndef CloneableVF3ParallelSubState_hpp
#define CloneableVF3ParallelSubState_hpp
...
/*----------------------------------------------------------
 * class CloneableVF3ParallelSubState
...
private:
	...
	// nodeID_t* predecessors;  //Previous node in the ordered sequence connected
	// to a node
...
public:
	...
	CloneableVF3ParallelSubState(
		...
		nodeID_t *order = NULL);
	CloneableVF3ParallelSubState(
		const CloneableVF3ParallelSubState &state, bool hard_copy = true);
	~CloneableVF3ParallelSubState();
...
/*----------------------------------------------------------
 * CloneableVF3ParallelSubState::CloneableVF3ParallelSubState(g1, g2)
...
CloneableVF3ParallelSubState<
	...
	CloneableVF3ParallelSubState(
		...
		uint32_t nclass,
		nodeID_t *order)
	: predecessors(ag1->NodeCount()), dir(ag1->NodeCount())
{
	assert(class_1 != NULL && class_2 != NULL);

	core_1 = new std::vector<nodeID_t>(ag1->NodeCount());
	core_2 = new std::vector<nodeID_t>(ag2->NodeCount());
	core_len_c = new std::vector<int32_t>(nclass);
	...
	for (i = 0; i < n2; i++) { (*core_2)[i] = NULL_NODE; }
...
/*----------------------------------------------------------
 * CloneableVF3ParallelSubState::CloneableVF3ParallelSubState(state)
 * Copy constructor.
 ---------------------------------------------------------*/
template<
	typename Node1,
...
CloneableVF3ParallelSubState<
	...
	EdgeComparisonFunctor>::
	CloneableVF3ParallelSubState(
		const CloneableVF3ParallelSubState &state, bool hard_copy)
	: predecessors(state.predecessors), dir(state.dir)
{
	if (hard_copy) {
		core_1 = new std::vector<nodeID_t>(*(state.core_1));
		core_2 = new std::vector<nodeID_t>(*(state.core_2));
		core_len_c = new std::vector<int32_t>(*(state.core_len_c));
		share_count = new int64_t;
		*share_count = 1;
	} else {
		core_1 = state.core_1;
...
template<
	...
	typename NodeComparisonFunctor,
	typename EdgeComparisonFunctor>
void CloneableVF3ParallelSubState<
	Node1,
	Node2,
	Edge1,
	Edge2,
...
template<
	...
	typename NodeComparisonFunctor,
	typename EdgeComparisonFunctor>
bool CloneableVF3ParallelSubState<
	Node1,
	Node2,
	Edge1,
	Edge2,
...
/*---------------------------------------------------------------
 * bool CloneableVF3ParallelSubState::IsFeasiblePair(node1, node2)
...
/*--------------------------------------------------------------
 * void CloneableVF3ParallelSubState::AddPair(node1, node2)
...
CloneableVF3ParallelSubState<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::~CloneableVF3ParallelSubState()
{
