...
/*
 * VF3P2
 * Parallel Matching Engine with local and global state stack (no look-free
 * stack)
 */
...
{
private:
	uint16_t ssrLimitLevelForGlobalStack;// all the states belonging to ssr
																			 // levels leq the this limit are put
																			 // inside the global stack
	uint16_t
		localStackLimitSize;// limit size for the local stack. All the exceeding
												// states are stored in the global stack
	std::vector<std::vector<VFState *>>
		localStateStack;// Local stack address by thread-id (ids are assigned by
										// the pool)
...
public:
	ParallelMatchingEngineWLS(
		...
		: ParallelMatchingEngine<VFState>(
			numThreads, storeSolutions, lockFree, cpu, visit),
...
private:
	...
	{
		if (
			thread_id == NULL_THREAD || s->CoreLen() <= ssrLimitLevelForGlobalStack
