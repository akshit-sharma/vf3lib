...
protected:
	...
	virtual void GetState(VFState **res, ThreadId thread_id)
	{
		*res = nullptr;
		...
		if (stackitem != nullptr) { *res = *(stackitem.get()); }
	}

	inline unsigned GetRemainingStates() { return globalStateStack->size(); }
	...
	bool ProcessState(VFState *s, ThreadId thread_id)
	{
		...
		while (s->NextPair(&n1, &n2, n1, n2)) {
			if (s->IsFeasiblePair(n1, n2)) { ExploreState(s, n1, n2, thread_id); }
		}
		return false;
	}

	virtual void
		ExploreState(VFState *s, nodeID_t n1, nodeID_t n2, ThreadId thread_id)
	{
		statesToBeExplored++;
...
public:
	...
	~ParallelMatchingEngine() { delete globalStateStack; }
	...
	bool FindAllMatchings(VFState &s)
	{
		...
		std::cout << "Pool Started: " << GetElapsedTime(start_time, pool_time)
							<< std::endl;
		// std::cout<<"Last Thread Stopped: "<<maxEOFTime<<std::endl;
		std::cout << "Pool Closed: " << GetElapsedTime(pool_time, eos_time)
							<< std::endl;
		std::cout << "Pool Closed: " << GetElapsedTime(eos_time, exit_time)
							<< std::endl;
#endif
		return true;
	}

	inline size_t GetThreadCount() const { return pool.size(); }
