 ...
 */
template<typename T1, typename T2> class EqualityComparator
{
public:
 ...
 * @see argloader.hpp
 */
template<typename Node, typename Edge> class ARGLoader
{
public:
	virtual ~ARGLoader() {}
 ...
 * @see argedit.hpp
 */
template<typename Node, typename Edge> class ARGraph
{
public:
 ...
 */
template<typename Node, typename Edge>
inline bool ARGraph<Node, Edge>::GetNodeIndex(
	nodeID_t n1, nodeID_t n2, nodeID_t &index) const
{
	unsigned long a, b, c;
	// NodeVec id = out[n1];
...
template<typename Node, typename Edge>
inline nodeID_t
	ARGraph<Node, Edge>::GetInEdge(nodeID_t node, uint32_t i, Edge &pattr) const
{
	assert(node < n);
	assert(i < in[node].size());
...
template<typename Node, typename Edge>
inline nodeID_t
	ARGraph<Node, Edge>::GetOutEdge(nodeID_t node, uint32_t i, Edge &pattr) const
{
	assert(node < n);
	assert(i < out[node].size());
 ...
 */
template<typename Node, typename Edge>
void ARGraph<Node, Edge>::VisitInEdges(
	nodeID_t node, edge_visitor vis, param_type param)
{
	assert(node < n);
	size_t i;
 ...
 */
template<typename Node, typename Edge>
void ARGraph<Node, Edge>::VisitOutEdges(
	nodeID_t node, edge_visitor vis, param_type param)
{
	assert(node < n);
	size_t i;
 ...
 */
template<typename Node, typename Edge>
void ARGraph<Node, Edge>::VisitEdges(
	nodeID_t node, edge_visitor vis, param_type param)
{
