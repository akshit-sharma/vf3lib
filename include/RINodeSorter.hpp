...
public:
	...
	bool operator<(const RISortingNode &rhs) const
	{
		...
		// Indeed a node that belongs to the neig set is supposed to be already
		// considered thus when it is insertend in the solution the unv_count has
		// not be decreased but the neig_counts

		if (used && rhs.used) return true;
		if (used && !rhs.used) return false;
		if (!used && rhs.used) return true;

		if (visCount > rhs.visCount) return true;
		if (visCount == rhs.visCount && neigCount > rhs.neigCount) return true;
		if (
			visCount == rhs.visCount && neigCount == rhs.neigCount
...
public:
	std::vector<nodeID_t> SortNodes(ARGraph<Node, Edge> *pattern)
	{
		...
		for (i = 0; i < nodeCount; i++) {
			nodes[i] =
				new RISortingNode(i, pattern->EdgeCount(i), pattern->OutEdgeCount(i));
		}
		...
		for (n = 0; n < nodeCount; n++) {
			...
			for (i = 0; i < out1_count; i++) {
				...
				if (!nodes[neigh]->IsUsed()) {
					...
					for (k = 0; k < out1_count_2; k++) {
						...
						if (
							neigh2 != node && !nodes[neigh2]->IsUsed()
			...
			candidate_it = min_element(
				nodes.begin(), nodes.end(), CompareCandidates<RISortingNode>());
		}
