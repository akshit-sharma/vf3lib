...
private:
	...
	std::map<nodeID_t, std::map<nodeID_t, Edge>>
		edges_map;// only to force undirected load
 ...
 */
template<typename Node, typename Edge>
StreamARGLoader<Node, Edge>::StreamARGLoader(
	std::istream &in, bool force_undirected)
	: force_undirected(force_undirected)
{
	...
	int64_t i;
	for (i = 0; i < cnt; i++) { readNode(in); }

	for (i = 0; i < cnt; i++) {
		int64_t ecount, j;
		ecount = readCount(in);
		for (j = 0; j < ecount; j++) readEdge(in);
	}
	if (force_undirected) {
		for (nodeID_t n1 = 0; n1 < cnt; n1++) {
			for (typename std::map<nodeID_t, Edge>::iterator iter =
						 edges_map[n1].begin();
					 iter != edges_map[n1].end();
					 ++iter) {
 ...
 */
template<typename Node, typename Edge>
void StreamARGLoader<Node, Edge>::readLine(std::istream &in, char *line)
{
	char *p;
	do {
		*line = '\0';
 ...
 */
template<typename Node, typename Edge>
int64_t StreamARGLoader<Node, Edge>::readCount(std::istream &in)
{
 ...
 */
template<typename Node, typename Edge>
void StreamARGLoader<Node, Edge>::readNode(std::istream &in)
{
	char line[MAX_LINE + 1];
	readLine(in, line);
	std::istringstream is(line);
	...
	if (id != this->NodeCount()) error("File format error\n  Line: %s", line);
 ...
 */
template<typename Node, typename Edge>
void StreamARGLoader<Node, Edge>::readEdge(std::istream &in)
{
	char line[MAX_LINE + 1];
	readLine(in, line);
	std::istringstream is(line);
...
/**
 * @brief Writes an ARGraph on a stream in a format readable by
 * StreamARGLoader.\n Relies on stream output operators for the Node and Edge
 * types.
 * @param [in] out Output stream.
 * @param [in] g ARGraph to write.
 */
template<typename Node, typename Edge>
void StreamARGLoader<Node, Edge>::write(
	std::ostream &out, ARGraph<Node, Edge> &g)
{
...
/**
 * @brief Writes an ARGLoader on a stream in a format readable by
 * StreamARGLoader.\n Relies on stream output operators for the Node and Edge
 * types.
 * @param [in] out Output stream.
...
template<typename Node, typename Edge>
void StreamARGLoader<Node, Edge>::write(
	std::ostream &out, ARGLoader<Node, Edge> &g)
{
	out << g.NodeCount() << std::endl;
