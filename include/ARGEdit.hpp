 ...
 *	Nodes are stored as a linked list and each node stores the list of its
 *out going edges.
 ...
 */
template<typename Node, typename Edge> ARGEdit<Node, Edge>::ARGEdit()
{
	count = 0;
	nodes = NULL;
	lastNode = NULL;
...
ARGEdit<Node, Edge>::ARGEdit(ARGraph<Node, Edge> &g)
{
	...
	for (n = 0; n < g.NodeCount(); n++) InsertNode(g.GetNodeAttr(n));
...
{
	nNode *n = nodes;
	if (lastNode != NULL && lastNode->id <= id) n = lastNode;

	while (n != NULL && n->id != id) n = n->next;
	if (n == NULL) error("Inconsistent data");
	return n->attr;
}
...
nodeID_t ARGEdit<Node, Edge>::GetOutEdge(nodeID_t id, uint32_t i, Edge *pattr)
{
	...
	if (
		lastEdge != NULL && lastEdge->from == id && lastEdge->pos >= 0
		&& lastEdge->pos <= i) {
		e = lastEdge;
		pos = e->pos;
	}
	...
	if (e == NULL) error("Inconsistent data");

	if (pattr != NULL) *pattr = e->attr;
	lastNode = n;
	lastEdge = e;
 ...
 * @description Insertion is expected to be in order to allow using binary
 * search while serching for an edge
...
void ARGEdit<Node, Edge>::InsertEdge(nodeID_t id1, nodeID_t id2, Edge &attr)
{
	...
	e->pos = -1;
	nNode *pn = nodes;
	if (lastNode != NULL && lastNode->id <= id1) { pn = lastNode; }
	while (pn != NULL && pn->id < id1) { pn = pn->next; }
...
void ARGEdit<Node, Edge>::DeleteNode(nodeID_t id)
{
	...
	if (p == NULL || !(p->id == id)) error("Bad param in ARGEdit::DeleteNode");
...
void ARGEdit<Node, Edge>::DeleteEdge(nodeID_t id1, nodeID_t id2)
{
	...
	if (pn == NULL || pn->id != id1) error("Bad param in ARGEdit::DeleteEdge");
