...
private:
	...
	uint32_t t1both_len, t2both_len, t1in_len, t1out_len, t2in_len,
		t2out_len;// Core nodes are also counted by these...
...
public:
	...
	inline bool IsGoal() const { return core_len == n1 && core_len == n2; };
	...
	: State(ag1->NodeCount(), ag2->NodeCount(), order, induced)
{
	...
	if (!in_1 || !in_2 || !out_1 || !out_2) error("Out of memory");
...
VF2State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::VF2State(const VF2State &state)
	: State(state)
{
	g1 = state.g1;
	g2 = state.g2;
...
VF2State<
	...
	Edge2,
	NodeComparisonFunctor,
	EdgeComparisonFunctor>::~VF2State()
{
	...
	NextPair(nodeID_t *pn1, nodeID_t *pn2, nodeID_t prev_n1, nodeID_t prev_n2)
{
	...
	if (t1both_len > core_len && t2both_len > core_len) {
		...
		uint32_t i = 0;
		while (i < n1 && core_1[prev_n1 = order[i]] != NULL_NODE) i++;
		if (i == n1) prev_n1 = n1;
	} else {
	...
	if (t1both_len > core_len && t2both_len > core_len) {
		...
		while (prev_n2 < n2 && core_2[prev_n2] != NULL_NODE) { prev_n2++; }
	}
	...
	EdgeComparisonFunctor>::IsFeasiblePair(nodeID_t node1, nodeID_t node2)
{
	...
	Edge1 eattr1;
	Edge2 eattr2;
	uint32_t termout1 = 0, termout2 = 0, termin1 = 0, termin2 = 0, new1 = 0,
					 new2 = 0;
	...
	EdgeComparisonFunctor>::BackTrack()
{
	...
	if (orig_core_len < core_len) {
		...
		for (i = 0; i < g2->OutEdgeCount(node2); i++) {
			int32_t other = g2->GetOutEdge(node2, i);
